...
inference_mode_list = ['Pretrained Voice', '3s Fast Clone', 'Cross-lingual Clone', 'Natural Language Control']
...
def main():
    with gr.Blocks() as demo:
        ...
        tts_text = gr.Textbox(label="Input Text", lines=1, value="I am a new generative speech model launched by the Tongyi Lab speech team, providing comfortable and natural speech synthesis capabilities.")
        with gr.Row():
            ...
            speed = gr.Number(value=1, label="Speed Adjustment (Only for Non-Stream Inference)", minimum=0.5, maximum=2.0, step=0.1)
            with gr.Column(scale=0.25):
                seed_button = gr.Button(value="\U0001F3B2")
                seed = gr.Number(value=0, label="Random Seed")

        with gr.Row():
            prompt_wav_upload = gr.Audio(sources='upload', type='filepath', label='Upload Prompt Audio File (Sample Rate >= 16kHz)')
            prompt_wav_record = gr.Audio(sources='microphone', type='filepath', label='Record Prompt Audio File')
        prompt_text = gr.Textbox(label="Input Prompt Text", lines=1, placeholder="Please enter the prompt text, which should match the prompt audio content. Automatic recognition is not supported yet...", value='')
        instruct_text = gr.Textbox(label="Input Instruct Text", lines=1, placeholder="Please enter the instruct text.", value='')
